    ...
    stages {        
       ...
       stage('Checkout') {
        ...
        stage('Standard Build') {
            ...
            def docker = load "${env.WORKSPACE}/utilities/docker/dockerBuild.groovy"
            jenkinsHelper= load "${env.WORKSPACE}/utilities/jenkins/jenkinsHelper.groovy"
            def shellScript = """
            git rev-parse --short HEAD
            """ 
            commitID= jenkinsHelper.mysh(shellScript) // Taking the commitID of the current git branch
